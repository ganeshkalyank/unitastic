...
const getPersonalizedMaterials = async () => {
    ...
    try {
        ...
        const semestersRef = collection(db, "semesters")
        const semesterQuery = query(semestersRef, where("name", "==", userData.data().semester))
        const semesterSnapshot = await getDocs(semesterQuery)
        const departmentRef = collection(db, "semesters", semesterSnapshot.docs[0].id, "depts")
        const departmentQuery = query(departmentRef, where("name", "==", userData.data().department))
        const departmentSnapshot = await getDocs(departmentQuery)
        return {...departmentSnapshot.docs[0].data(), semester: userData.data().semester }
    } catch (error) {
        return {
